...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Modal Package Selection Logic
            const modal = document.getElementById('staticBackdrop');
            if (modal) {
                modal.addEventListener('show.bs.modal', function(event) {
                    const button = event.relatedTarget;
                    const packageId = button.getAttribute('data-package-id');
                    const select = modal.querySelector('select[name="package_id"]');
                    ...
                    if (select) {
                        if (packageId) {
                            ...
                            // 1. Try direct value match first (unlikely if IDs are integers)
                            // 2. Try text match (case-insensitive)
                            Array.from(select.options).forEach(option => {
                                if (option.text.toLowerCase().includes(packageId.toLowerCase())) {
                                    select.value = option.value; // Set value to the correct ID
                                    found = true;
                                }
                            });
                            
                            // If found, lock the select
                            if(found){
                                select.style.pointerEvents = 'none';
                                select.style.backgroundColor = '#e9ecef'; // Readonly look
                                select.tabIndex = -1; // Prevent keyboard focus
                            } else {
                                // Fallback: unlock if not found (or handle error)
                                select.style.pointerEvents = 'auto';
                                select.style.backgroundColor = '';
                                select.removeAttribute('tabindex');
                            }
                        } else {
                            // No package selected (e.g. general button), reset and unlock
