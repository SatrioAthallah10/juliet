    ...
    <div class="payment-container">
        ...
        <div class="info-section">
            <div class="info-card">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $school->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Email</span>
                    <span class="info-value">{{ $school->support_email ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">No. Telepon</span>
                    <span class="info-value">{{ $school->support_phone ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Alamat</span>
                    <span class="info-value">{{ $school->address ?? 'N/A' }}</span>
                </div>
            </div>
            ...
            <div class="amount-card">
                ...
                <div class="amount">Rp {{ number_format($payment_amount ?? 0, 0, ',', '.') }}</div>
        ...
        <div class="action-section">
            <form action="{{ route('payment.process') }}" method="POST" id="payment-form">
                ...
                <input type="hidden" name="amount" value="{{ $payment_amount ?? 0 }}">
    ...
    <script>
        ...
        if (distance < 0) {
            ...
            Swal.fire({
                ...
                window.location.href = '{{ route("registration.cancel", ["school_id" => $school->id ?? ""]) }}';
            });
            return;
        }
...
startCountdown();
        ...
        html: `
            ...
            <p class="mt-3"><strong>Total: Rp {{ number_format($payment_amount ?? 0, 0, ',', '.') }}</strong></p>
        `,
        icon: 'question',
